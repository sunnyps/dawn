...
HardwareJpegDecoder::HardwareJpegDecoder(wgpu::Device device)
    : device_(device) {
  wgpu::ShaderModule shader = utils::CreateShaderModule(device_, R"(
      [[block]] struct Params {
        width: u32;
        height: u32;
      };
...
HardwareJpegDecoder::~HardwareJpegDecoder() = default;
...
wgpu::TextureView HardwareJpegDecoder::Decode(std::vector<uint8_t> data, int* width, int* height) {
    ...
    *height = static_cast<int>(jpeg.height);
    ...
    wgpu::Buffer uniforms =
        utils::CreateBufferFromData(device_, wgpu::BufferUsage::Uniform, {jpeg.width, jpeg.height});
    wgpu::Buffer y_dct_data = utils::CreateBufferFromData(
        device_, y.data(), y.size() * sizeof(int32_t) * 64, wgpu::BufferUsage::Storage);
    wgpu::Buffer cr_dct_data = utils::CreateBufferFromData(
        device_, cr.data(), cr.size() * sizeof(int32_t) * 64, wgpu::BufferUsage::Storage);
    wgpu::Buffer cb_dct_data = utils::CreateBufferFromData(
        device_, cb.data(), cb.size() * sizeof(int32_t) * 64, wgpu::BufferUsage::Storage);
    ...
    wgpu::BindGroup bg0 = utils::MakeBindGroup(device_, pipeline_.GetBindGroupLayout(0),
                                               {
                                                   {0, uniforms},
                                                   {1, texture.CreateView()},
                                               });
    wgpu::BindGroup bg1 = utils::MakeBindGroup(device_, pipeline_.GetBindGroupLayout(1),
                                               {
                                                   {0, y_dct_data},
                                                   {1, cb_dct_data},
                                                   {2, cr_dct_data},
